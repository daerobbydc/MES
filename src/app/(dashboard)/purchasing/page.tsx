...
import {
  ShoppingCart, Plus, Search, Eye, CheckCircle, Clock, Truck, FileText, X,
} from "lucide-react";
...
export default function PurchasingPage() {
  ...
  const [lines, setLines] = useState<{ productId: string; materialCode: string; quantity: number; unitPrice: number }[]>([]);
  const [lineDraft, setLineDraft] = useState({ productId: "", materialCode: "", quantity: 1, unitPrice: 0 });
  ...
  const addLine = () => {
    if (!lineDraft.productId || !lineDraft.materialCode || lineDraft.quantity <= 0) return;
    setLines([...lines, { ...lineDraft }]);
    setLineDraft({ productId: "", materialCode: "", quantity: 1, unitPrice: 0 });
  };
  ...
  const handleCreate = async () => {
    if (!form.supplierId || !form.expectedDate) {
      setError("Supplier and Expected Date are required");
      return;
    }
    if (lines.length === 0) {
    ...
    try {
      const res = await fetch("/api/purchasing/orders", {
        ...
        body: JSON.stringify({
          ...
          notes: form.notes,
          items: lines.map(l => ({ productId: l.productId, materialCode: l.materialCode, quantity: l.quantity, unitPrice: l.unitPrice })),
        }),
      });
      const json = await res.json();
      if (json.success) {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-sm text-surface-500 font-medium">Loading purchase orders...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-surface-900 tracking-tight">Purchasing</h1>
          <p className="text-surface-500 text-sm mt-0.5">Manage purchase orders and receiving</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "Total POs", value: orders.length, color: "text-surface-900", bg: "bg-primary-50", iconText: "text-primary-600", icon: ShoppingCart },
          { label: "Pending", value: orders.filter(o => o.status === "PENDING" || o.status === "SENT").length, color: "text-amber-600", bg: "bg-amber-50", iconText: "text-amber-600", icon: Clock },
          { label: "Received", value: orders.filter(o => o.status === "RECEIVED" || o.status === "COMPLETED").length, color: "text-emerald-600", bg: "bg-emerald-50", iconText: "text-emerald-600", icon: CheckCircle },
        ].map((s, i) => (
        ...
        <table className="w-full">
          <thead>
            <tr className="table-header">
              ...
              <th className="px-5 py-3.5 text-left">Expected</th>
              <th className="px-5 py-3.5 text-center">Status</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((po, idx) => (
              <tr key={po.id} className="border-b border-surface-100/50 hover:bg-surface-50/60 transition-colors animate-in fade-in" style={{ animationDelay: `${idx * 0.02}s` }}>
                <td className="px-5 py-4 font-mono text-sm font-bold text-primary-600">{po.orderNumber}</td>
                <td className="px-5 py-4 text-sm font-medium text-surface-800">{po.supplier?.name || "-"}</td>
                <td className="px-5 py-4 text-sm font-bold text-surface-900 text-right">Rp {(po.totalAmount || 0).toLocaleString()}</td>
                <td className="px-5 py-4 text-sm text-surface-600">{po.expectedDate ? new Date(po.expectedDate).toLocaleDateString() : "-"}</td>
                <td className="px-5 py-4 text-center">
                  <span className={`status-badge ${
                    po.status === "COMPLETED" || po.status === "RECEIVED" ? "status-done" :
                    po.status === "SENT" ? "status-in-progress" :
                    po.status === "PARTIAL" ? "status-idle" : "status-planned"
                  }`}>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="text-center py-16 text-surface-400">
                <ShoppingCart size={40} className="mx-auto mb-3 text-surface-300" />
      ...
      {showModal && (
        <div className="fixed inset-0 bg-surface-900/40 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-float w-full max-w-lg p-7 animate-in scale-in duration-300 border border-white/60">
            <h3 className="text-lg font-bold text-surface-900 mb-5">New Purchase Order</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="label">Expected Date <span className="text-red-500">*</span></label>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  <select value={lineDraft.productId} onChange={e => {
                    const prod = products.find(p => p.id === e.target.value);
                    setLineDraft({ ...lineDraft, productId: e.target.value, materialCode: prod?.code || "" });
                  ...
                  <input type="number" min={0} value={lineDraft.unitPrice || ""} onChange={e => setLineDraft({ ...lineDraft, unitPrice: parseFloat(e.target.value) || 0 })} className="input-field w-28 text-sm" placeholder="Unit Cost" />
